...
剪映 11.1.0.14287 已安装。真实草稿已写入剪映用户草稿根目录，并被剪映自身的 `root_meta_info.json` 与 `draft_acion_watch.json` 索引，日志记录 `copy_draft_external`、`suc:true`。用户提供的剪映 GUI 截图进一步确认草稿已打开、预览画面正常、时间线上存在两个边界清晰且可单独选中的 2 秒片段；剪映应用内验收已通过。
...
- GUI 人工验收：草稿已打开，预览画面正常；时间线显示两个相邻、边界清晰、均为 2 秒的独立片段，第二段处于单独选中状态，无丢失媒体提示。
