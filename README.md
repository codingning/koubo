...
口播生成前会先锁定一个“普通人怎样使用 AI 得到具体结果”的话题，再只读调研用户指定参考账号和抖音同题视频；至少一条完整视频经过本地转录或已核验研究后，才会生成默认 2—3 分钟完整版。代码、文件名和 Git 默认只作幕后证据，正文按“结果先行—AI输入—第一版—具体返修—最终边界”组织。拍后默认进入 `visual-director-v4`：同题风格分析、ASR与内容拆解、关键帧、关键帧审核、动态样片审核、2K全片与QA。六步参数和提示词都可在网页修改；没有修改就使用版本化默认值。**不会自动发布、评论或连接平台账号。**
