...
## 当前 14 天产品验证（2026-08-17—2026-08-31）

当前最高优先任务是验证“本地可追溯的创作者内容证据库”，首版采用边界固定的建库冲刺，不先扩建 SaaS。目标用户是每月至少发布 4 条、已有 100 条以上收藏或素材，并且最近曾因找不到原来源而返工的中文知识型创作者、讲师或顾问。

内部 V0 已用 62 条本人真实收藏跑通：本地证据库 → 3 个有来源的选题包 → 1 个内容大纲。真实收藏、媒体、转录和账号信息只保存在 Git 忽略的 `.runtime/`，仓库只提交合同、构建器、验证器和测试。内部结果获得用户验收前，不启动招募、报价、私信或收款。

- [产品验证合同](docs/product-validation/creator-evidence-library-v1/README.md)
- [内部 V0 验收标准](docs/product-validation/creator-evidence-library-v1/INTERNAL_V0.md)
- [本地构建与验证说明](scripts/product-v0/README.md)
