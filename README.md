...
口播生成前会先锁定明确的 AI 话题，再只读调研用户指定参考账号和抖音同题视频；至少一条完整视频经过本地转录或已核验研究后，才会生成默认 2—3 分钟完整版。证据汇总、语义剪辑、字幕、重点卡片、声音处理、画幅适配、四画幅文字封面、渲染、QA 和返修版本记录由本地工作流自动完成。**不会自动发布、评论或连接平台账号。**
...
- 参考研究优先读取 `config/reference_creators.json` 中用户指定的两个抖音账号，并补充同题搜索；
