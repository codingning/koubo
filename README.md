...
上传拍摄原片
  ...
  → 自动拆解3—6个视觉节点，生成本地零费用候选并登记参考视频候选
  → 用户在素材审核板逐条批准、拒绝、替换和调整时间；全部决定后才渲染
  → FFmpeg 删除停顿/重复区间、加入30ms切点淡化
  → 默认按16:9科技感横版重新编排真人口播、工作台录屏、前后对比、画中画与章节信息层，再加入逐字字幕、基础滤波和-16 LUFS；其他画幅可按需派生
...
- `video/server.mjs`：仅监听 `127.0.0.1:8787` 的 v3 本地服务。
...
2026-07-21 的 v3 端到端测试已验证：计划后暂停等待素材审核、候选逐条批准/拒绝、本地信息卡实际合成、外部视频稿件披露拦截、授权测试片段画中画、来源署名、manifest `composited` 状态、素材QA和最终审核门禁。
